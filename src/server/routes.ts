...
interface RouteContext {
  ...
  cycleLogger?: StructuredCycleLogger;
  cycleMetrics?: CycleMetricsCollector;
}
...
const SSE_WARN_CONNECTION_THRESHOLD = Math.floor(
  SSE_MAX_CONNECTIONS_PER_ENDPOINT * 0.8,
);
...
function reserveSseConnection(
  ...
  const endpointKey =
    normalizedEndpoint.length > 0 ? normalizedEndpoint : "unknown-sse-endpoint";
  const connections =
    sseConnectionsByEndpoint.get(endpointKey) ?? new Set<symbol>();
  ...
  if (
    previousConnections <= SSE_WARN_CONNECTION_THRESHOLD &&
    activeConnections > SSE_WARN_CONNECTION_THRESHOLD
  ) {
...
export function createSseStream(
  ...
  if (!req) {
    throw new TypeError(
      "createSseStream requires an IncomingMessage instance.",
    );
  }
  if (!res) {
...
async function resolveCurrentTaskTitle(
  taskStore: TaskStore,
  taskId: string | null,
): Promise<string | null> {
  if (!taskId) {
...
route("GET", "/api/metrics", async (_req, res, ctx) => {
  const rawProjectPath = (ctx.loop as unknown as { projectPath?: unknown })
    .projectPath;
  const projectPath =
...
route("GET", "/api/status/stream", async (req, res, ctx) => {
  ...
  const pushStatus = async (snapshot: StatusSseSnapshot): Promise<boolean> => {
    const payload: StatusSsePayload = {
      ...snapshot,
      currentTaskTitle: await resolveCurrentTaskTitle(
        ctx.taskStore,
  ...
  const wroteInitialStatus = await pushStatus(ctx.loop.getStatusSnapshot());
...
route("GET", "/api/tasks", async (req, res, ctx) => {
  ...
  const status = statusParam
    ? (statusParam
        .split(",")
        .filter(Boolean) as import("../memory/types.js").TaskStatus[])
    : undefined;
  const result = await ctx.taskStore.listTasksPaged(
    ctx.config.projectPath,
    page,
    pageSize,
    status,
...
route("POST", "/api/control/scan", async (req, res, ctx) => {
  if (ctx.loop.isRunning()) {
    json(
      res,
      { error: "Cannot trigger manual scan while patrol is running" },
      409,
    );
    return;
  }
...
route("GET", "/api/logs", async (req, res, ctx) => {
  ...
  if (follow) {
    ...
    // Replay recent history so the client sees entries immediately
    for (const entry of log.getRecentEntries(100)) {
    ...
    json(res, log.getRecentEntries(100));
  }
});
...
route("POST", "/api/patrol/start", async (_req, res, ctx) => {
  if (!ctx.patrolManager) {
    json(res, { error: "Patrol manager not available" }, 503);
    return;
  }
  try {
...
route("GET", "/api/plans", async (req, res, ctx) => {
  ...
  const status = url.searchParams.get("status") as
    | import("../memory/types.js").PlanReviewStatus
    | null;
  const drafts = await ctx.taskStore.listPlanDrafts(
    ctx.config.projectPath,
    status ?? undefined,
  );
  json(res, drafts);
...
// --- Cycle observer routes ---

route("GET", "/api/cycle/metrics", async (_req, res, ctx) => {
  if (!ctx.cycleMetrics) {
    json(res, { error: "Cycle metrics not available" }, 503);
    return;
  }
  json(res, ctx.cycleMetrics.getMetrics());
});

route("GET", "/api/cycle/entries", async (req, res, ctx) => {
  if (!ctx.cycleLogger) {
    json(res, { error: "Cycle logger not available" }, 503);
    return;
  }
  const url = new URL(req.url ?? "", `http://${req.headers.host}`);
  ...
  const entries = ctx.cycleLogger.getEntries();
  json(res, entries.slice(-limit));
});
...
function validateCreateTaskBody(
  body: unknown,
  ...
  if (normalizedDescription.length > MAX_TASK_DESCRIPTION_LENGTH) {
    return {
      ok: false,
      error: `description must be at most ${MAX_TASK_DESCRIPTION_LENGTH} characters.`,
    };
  }
