...
import type { RegisteredObservers } from "../core/observers/index.js";
...
export class Server {
  ...
  constructor(
    ...
    private observers?: RegisteredObservers,
    ...
    const ctx = {
      ...
      costTracker,
      config,
      patrolManager: this.patrolManager,
      planChat: this.planChat,
      cycleLogger: this.observers?.logger,
      cycleMetrics: this.observers?.metrics,
    };
    ...
    this.server = createServer(
      async (req: IncomingMessage, res: ServerResponse) => {
        ...
        try {
          if (
            this.shouldApplyRateLimit(req) &&
            !this.rateLimiter(this.getClientIp(req))
          ) {
            req.resume();
            res.writeHead(429, {
