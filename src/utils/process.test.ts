...
import { parseJsonlEvents, runProcess, spawnWithJsonl, type JsonlEvent } from './process.js';

describe('runProcess', () => {
  it('returns stdout and exitCode for a successful process', async () => {
    const result = await runProcess(
      process.execPath,
      ['-e', "const fs=require('node:fs');fs.writeSync(1,'hello\\n');"],
    );
