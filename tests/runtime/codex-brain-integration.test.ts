...
import { CodexSdkRuntime } from "../../src/runtime/CodexSdkRuntime.js";
import { runBrainThink } from "../../src/core/phases/brainThink.js";
import { Config } from "../../src/config/Config.js";

// Determine skip reason (if any) without process.exit so this file
// is safe to include in multi-file `node --test` runs.
let skipReason: string | undefined;
if (!process.env.RUN_CODEX_INTEGRATION) {
  skipReason =
    "Set RUN_CODEX_INTEGRATION=1 to run real API integration tests";
} else {
  const probe = new CodexSdkRuntime();
  if (!(await probe.isAvailable())) {
    skipReason = "Codex SDK binary not available";
  }
}

// Model: prefer env override, fall back to default.
const CODEX_MODEL = process.env.CODEX_TEST_MODEL ?? "gpt-5.4";

describe(
  "CodexSdkRuntime brain integration (real API)",
  { timeout: 120_000, skip: skipReason },
  () => {
    // runtime is only used when not skipped
