...
import { TaskQueue, type EnqueueResult } from "../../src/core/TaskQueue.js";
import type { PlanTask, TaskPlan } from "../../src/core/types.js";

type SimilarTaskMatcher = (
  projectPath: string,
  description: string,
) => Task | null | Promise<Task | null>;
type CooldownMatcher = (
  projectPath: string,
  description: string,
...
interface StoreMockFixture {
  ...
  findSimilarCompletedTaskCalls: Array<{
    projectPath: string;
    description: string;
  }>;
  hasRecentlyFailedSimilarCalls: Array<{
    projectPath: string;
    description: string;
  }>;
}
...
function createStoreMock(options: StoreMockOptions = {}): StoreMockFixture {
  ...
  const findSimilarTaskCalls: Array<{
    projectPath: string;
    description: string;
  }> = [];
  const findSimilarCompletedTaskCalls: Array<{
    projectPath: string;
    description: string;
  }> = [];
  const hasRecentlyFailedSimilarCalls: Array<{
    projectPath: string;
    description: string;
  }> = [];
  const hasCustomEmptyNextValue = Object.prototype.hasOwnProperty.call(
    options,
    "emptyNextValue",
  );
  ...
  const store = {
    hasRecentlyFailedSimilar: async (
      projectPath: string,
      description: string,
    ) => {
    ...
    findSimilarCompletedTask: async (
      projectPath: string,
      description: string,
    ) => {
    ...
    updateTask: async (taskId: string, updates: unknown) => {
      ...
      Object.assign(task, updates as Partial<Task>, {
        updated_at: new Date("2026-02-01T00:01:00.000Z"),
      });
    },
    getNextTask: async (projectPath: string) => {
      ...
      const queued = tasks
        .filter(
          (task) =>
            task.project_path === projectPath && task.status === "queued",
        )
        .filter((task) =>
          task.depends_on.every((depId) =>
            tasks.some(
              (candidate) =>
                candidate.id === depId && candidate.status === "done",
            ),
          ),
        )
        .sort(
...
async function withCapturedInfoLogs<T>(
  fn: (messages: string[]) => Promise<T>,
): Promise<T> {
...
async function withCapturedWarnLogs<T>(
  fn: (messages: string[]) => Promise<T>,
): Promise<T> {
  const messages: string[] = [];
...
test("enqueue adds task and logs", async () => {
  ...
  const result = await withCapturedInfoLogs(async (messages) => {
    capturedLogs = messages;
    return queue.enqueue("/repo", plan);
  });

  assert.deepEqual(result.queuedIds, [
    "task-1",
    "task-2",
    "task-3",
    "task-4",
  ...
  assert.deepEqual(
    ...
    [[], ["task-1"], ["task-1"], ["task-2", "task-3"], [], ["task-5"]],
  );
  ...
  assert.equal(
    capturedLogs.filter((message) => message.startsWith("Queued task")).length,
    6,
  );
});
...
test("enqueue dedup — queued/active task still hard-blocks", async () => {
  ...
  assert.equal(
    capturedLogs.some((message) =>
      message.includes("Skipping duplicate task:"),
    ),
    true,
  );
});
...
test("enqueue recurrence — similar done task exists, new task created with metadata", async () => {
  ...
  const result = await withCapturedWarnLogs(async (messages) => {
    capturedWarnLogs = messages;
    return queue.enqueue("/repo", plan);
  });

  assert.deepEqual(result.queuedIds, ["task-1"]);
  assert.deepEqual(result.pendingReviewIds, []);
  assert.equal(fixture.hasRecentlyFailedSimilarCalls.length, 1);
  assert.equal(fixture.findSimilarTaskCalls.length, 1);
  ...
  assert.equal(
    capturedWarnLogs[0]?.includes(
      'Recurring issue detected: "Fix flaky migration" — similar to completed task done-1',
    ),
    true,
  );
});
...
test("enqueue dedup — done task no longer hard-blocks", async () => {
  ...
  const plan: TaskPlan = {
    ...
    tasks: [
      createPlanTask({
        id: "T-reopen",
        description: "Investigate latency regression",
      }),
    ],
  };

  const result = await queue.enqueue("/repo", plan);
...
test("enqueue separates queued and pending_review tasks in result", async () => {
  const fixture = createStoreMock();
  const queue = new TaskQueue(fixture.store);
  const plan: TaskPlan = {
    reasoning: "Mix of queued and review-gated tasks",
    tasks: [
      createPlanTask({
        id: "T-normal",
  ...
  assert.deepEqual(result.queuedIds, ["task-1", "task-3"]);
  assert.deepEqual(result.pendingReviewIds, ["task-2"]);
  // pending_review task should have 2 updateTask calls (subtasks + status change)
  const reviewUpdates = fixture.updateTaskCalls.filter(
    (call) => call.taskId === "task-2",
  );
  assert.equal(reviewUpdates.length, 2);
  assert.deepEqual(
    (reviewUpdates[1].updates as Record<string, unknown>).status,
    "pending_review",
  );
});
...
test("enqueue cooldown — hasRecentlyFailedSimilar returns true and task is skipped", async () => {
  ...
  const plan: TaskPlan = {
    ...
    tasks: [
      createPlanTask({
        id: "T-cooldown",
        description: "Retry flaky migration",
      }),
    ],
  };
  ...
  assert.equal(
    capturedLogs.some((message) =>
      message.includes("Skipping task (cooldown):"),
    ),
    true,
  );
});
...
test("getAll returns combined queued+blocked tasks", async () => {
  ...
  assert.deepEqual(
    ...
    ["queued-task", "blocked-task"],
  );
  assert.deepEqual(fixture.listTasksCalls, [
    { projectPath: "/repo", status: undefined },
  ]);
});
...
test("getQueued filters by status=queued", async () => {
  ...
  assert.deepEqual(fixture.listTasksCalls, [
    { projectPath: "/repo", status: "queued" },
  ]);
});
...
test("getBlocked filters by status=blocked", async () => {
  ...
  assert.deepEqual(
    ...
    ["blocked-dependency", "blocked-standalone"],
  );
  assert.deepEqual(fixture.listTasksCalls, [
