...
import { collectResult } from "../../src/bridges/sdkMessageCollector.js";
import type {
  SDKMessage,
  SDKResultSuccess,
  SDKResultError,
...
describe("collectResult", () => {
  ...
  it("synthesizes error when is_error=true but subtype=success (empty errors)", async () => {
    const result = await collectResult(
      toAsyncGen([makeSuccessResult({ is_error: true })]),
    );
    assert.strictEqual(result.isError, true);
    assert.strictEqual(result.exitCode, 0);
    assert.strictEqual(result.errors.length, 1);
    assert.ok(result.errors[0].includes("is_error=true"));
    assert.ok(result.errors[0].includes("subtype=success"));
  });
