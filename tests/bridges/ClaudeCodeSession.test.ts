...
import {
  ...
  type SessionResult,
  type StreamEvent,
} from "../../src/bridges/ClaudeCodeSession.js";
import type {
  SDKMessage,
  SDKResultSuccess,
  SDKResultError,
...
describe("ClaudeCodeSession", () => {
  ...
  describe("error path", () => {
    ...
    it("returns exitCode 1 when query throws an error", async () => {
      const session = new ClaudeCodeSession(
        {},
        createThrowingQuery(new Error("Network failure")),
      );
      const result = await session.run("test", baseOpts);
  ...
  describe("re-entrancy safety", () => {
    it("rejects concurrent run() calls", async () => {
      ...
      const first = session.run("first", { ...baseOpts, timeout: 5000 });

      // Attempt second run while first is active
      await assert.rejects(() => session.run("second", baseOpts), {
        message: /not re-entrant/,
      });

      // Clean up: kill the first run
      session.kill();
      await first;
    });

    it("kill() only affects the active call, not a subsequent one", async () => {
      const { queryFn: hangQuery } = createHangingQuery();
      const session = new ClaudeCodeSession({}, hangQuery);

      // Start and kill first run
      const first = session.run("first", { ...baseOpts, timeout: 5000 });
      await new Promise((r) => setTimeout(r, 10));
      session.kill();
      const r1 = await first;
      assert.strictEqual(r1.exitCode, -2, "first run should be killed");

      // Second run should succeed independently (no leftover killed state)
      const successQuery = createMockQuery([makeSuccessResult()]);
      const session2 = new ClaudeCodeSession({}, successQuery);
      const r2 = await session2.run("second", baseOpts);
      assert.strictEqual(
        r2.exitCode,
        0,
        "second run should not inherit killed state",
      );
    });

    it("kill() on idle session is a no-op", () => {
      const session = new ClaudeCodeSession(
        {},
        createMockQuery([makeSuccessResult()]),
      );
