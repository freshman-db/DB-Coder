...
async function withMockFetch(
  ...
  try {
    await fn(new Client(18801, "127.0.0.1", "secret-token"));
  } finally {
...
test("Client adds bearer token to log stream requests", async () => {
  ...
  try {
    const client = new Client(18801, "127.0.0.1", "secret-token");
...
test("stale memory methods do not exist on Client", () => {
  ...
  assert.equal(
    "searchMemory" in client,
    false,
    "searchMemory should be removed",
  );
